# -*- coding: utf-8 -*-
"""
Created on Tue Apr 29 10:34:45 2025
...
######################################################################################################################################
######################################################################################################################################
#####################################     QUADRATIC OPTIMIZATION ( NORMALIZED )       #############################################
######################################################################################################################################
######################################################################################################################################
######################################################################################################################################
import pandas as pd
...
P_wg = np.diag(1/consumption_wg **2)
P_dairy = np.diag(1/consumption_dairy **2)
P_fish = np.diag(1/consumption_fish **2)
P_fruit = np.diag(1/consumption_fruit **2)
P_veg = np.diag(1/consumption_veg **2)
P_meat = np.diag(1/consumption_meat **2)
P_nut = np.diag(1/consumption_nut **2)
P_legume = np.diag(1/consumption_legume **2)
P_misc = np.diag(1/consumption_misc **2)
...
prob.solve(solver=cp.ECOS, abstol=1e-5, reltol=1e-5)
print("Status:", prob.status)

#cp.pos(slope_meat * (20 - tmrel_meat) + intercept_meat).value -    cp.pos(slope_meat *(16 - 100)+ intercept_meat ).value
###################################### PRINT ##########################################################
...
burden_optimal =  {
    ...
    "BURDEN": [burden_ihd[0][0], burden_crc[0][0], burden_t2d[0][0], burden_stroke, burden_hyper]
}
